# Character table

This table lists the main character mapping, for more details and complex cases, see [doc.md](doc.md).

## Consonnants

| twktt | Khmer | Lao   | Thai  | Tham  | Khom  |
| :---: | :---: | :---: | :---: | :---: | :---: | 
| `ka`    | ក     | ກ     | ก     | (insert image) | (insert image) | 
| `kha`   | ខ     | ຂ     | ข     |  |  | 
| `ḱha`   |       |       | ฃ     |  |  | 
| `ga`    | គ     | ຄ     | ค     |  |  | 
| `ǵa`    |       |       | ฅ     |  |  | 
| `gha`   | ឃ     |       | ฆ     |  |  | 
| `ṅa`    | ង     | ງ     |  ง    |  |  | 
| `ca`    | ច     | ຈ     |  จ    |  |  | 
| `cha`   | ឆ     |       | ฉ     |  |  | 
| `ja`    | ជ     | ຊ     | ช     |  |  | 
| `j́a`    |       |       | ซ     |  |  | 
| `jha`   | ឈ     |      | ฌ     |  |  | 
| `ña`    | ញ     |      | ญ     |  |  | 
| `ṭa`    | ដ     |      | ฎ     |  |  | 
| `ṭha`   | ឋ     |      | ฐ     |  |  | 
| `ḍa`    | ឌ     |      | ฑ     |  |  | 
| `ḍha`   | ឍ     |      | ฒ     |  |  | 
| `ṇa`    | ណ     |      | ณ     |  |  | 
| `ta`    | ត     | ດ     | ด     |  |  | 
| `t́a`    |      | ຕ     | ต     |  |  | 
| `tha`   | ថ     | ຖ     | ถ     |  |  | 
| `da`    | ទ     | ທ     | ท     |  |  | 
| `dha`   | ធ     |      | ธ     |  |  | 
| `na`    | ន     | ນ     | น     |  |  | 
| `pa`    | ប     | ບ     | บ     |  |  | 
| `ṕa`    |      | ປ     | ป     |  |  | 
| `pha`   | ផ     | ຜ     | ผ     |  |  | 
| `ṕha`   |      | ຝ     | ฝ     |  |  | 
| `ba`    | ព     | ພ     | พ     |  |  | 
| `b́a`    |      | ຟ     | ฟ     |  |  | 
| `bha`   | ភ     |      | ภ     |  |  | 
| `ma`    | ម     | ມ     | ม     |  |  | 
| `ya`    | យ     | ຍ     | ย     |  |  | 
| `ýa`    |      | ຢ     |      |  |  | 
| `ra`    | រ     | ຣ     | ร     |  |  | 
| `la`    | ល     | ລ     | ล     |  |  | 
| `va`    | វ     | ວ     | ว     |  |  | 
| `śa`    | ឝ     |      | ศ     |  |  | 
| `ṣa`    | ឞ     |      | ษ     |  |  | 
| `sa`    | ស     | ສ     | ส     |  |  | 
| `ha`    | ហ     | ຫ     | ห     |  |  | 
| `ḷa` (ḷ followed by vowel)    | ឡ     |      | ฬ     |  |  | 
| `h́a`    |      | ຮ     | ฮ     |  |  |

## Independend vowels

| twktt | Khmer | Lao   | Thai  | Tham  | Khom  |
| :---: | :---: | :---: | :---: | :---: | :---: | 
| `'a`    | អ     | ອ     | อ     |  |  |
| `aa`    |      | ອອ     | ออ     |  |  | 
| `I`    | ឥ     |      |      |  |  | 
| `Ī`    | ឦ    |      |      |  |  | 
| `U`    | ឧ     |      |      |  |  | 
| `Ū`    | ឩ     |      |      |  |  | 
| `û`    | ឨ     |      |      |  |  | 
| `ṛ`    | ឫ     |      | ฤ     |  |  | 
| `ṝ`    | ឬ     |      | ฤๅ     |  |  | 
| `ḷ` (not followed by vowel)   | ឭ     |      |  ฦ    |  |  | 
| `ḹ`    | ឮ     |      | ฦๅ     |  |  | 
| `E`    | ឯ     |      |      |  |  | 
| `Ai` (?)    | ព     |      |      |  |  | 
| `O`    | ឱ    |      |      |  |  | 
| `Au`    | ឳ     |      |      |  |  | 
| `Uv` (?)    | ឪ     |      |      |  |  |
| `Ụ` or `Ọ` (?)    | ឲ     |      |      |  |  |

## Dependent vowels

Exemplified on អ / ອ / อ:

| twktt | Khmer | Lao   | Thai  | Tham  | Khom  |
| :---: | :---: | :---: | :---: | :---: | :---: | 
| `ā`    | អា     | ອາ     | อา     |  |  | 
| `i`    | អិ     |      |      |  |  | 
| `ī`    | អី     |      |      |  |  | 
| `ị̄ `   | អឹ     |      |      |  |  | 
| `u`    | អុ     |      |      |  |  | 
| `ū`    | អូ (or variant with two u)    |      |      |  |  | 
| `uo` (what does "va, (svat)" mean?)    | អួ     |      |      |  |  | 
| `ö`    | អើ     |      |      |  |  | 
| `īö`    | អឿ     |      |      |  |  | 
| `ie`    | អៀ    |      |      |  |  | 
| `e`    | អេ     |      |      |  |  | 
| `è`    | អែ     |      |      |  |  | 
| `ai`    | អៃ     |      |      |  |  | 
| `aï`    |      |      |      |  |  | 
| `o`    | អោ     |      |      |  |  | 
| `au`    | អៅ     |      |      |  |  | 


## Diacritics

Exemplified on អ / ອ / อ:

| twktt | Khmer | Lao   | Thai  | Tham  | Khom  |
| :---: | :---: | :---: | :---: | :---: | :---: | 
| `:`    | អៈ     |      |      |  |  | 
| `ṃ`    | អំ     |      |      |  |  | 
| `ḥ`    | អះ     |      |      |  |  | 
| `ă`   | អ័     |      |      |  |  | 
| `ā̆`   | អា័     |      |      |  |  | 
| `'` (warning: we already use `'` for អ?)   | អ់     |      |      |  |  | 
| `"`    | អ៉    |      |      |  |  | 
| `ŕ`    | អ៌     |      |      |  |  | 
| `ˆ`    | អ?     |      |      |  |  | 
| ()?    | អ៍     |      |      |  |  | 
| `₁`   |      |      |      |  |  | 
| `₂`   |      |      |      |  |  | 
| `₃`   |      |      |      |  |  | 
| `₄`   | អ៎     |      |      |  |  |
| `á` (not possible with other vowels)    | អ៏     |      |      |  |  |  



Note: can we find another solution for `()`?

## Punctuation

A space in twktt represents a word break, which is not explicit in Khmer script; it does not represent a space. There's no 

| twktt | Khmer | Lao   | Thai  | Tham  | Khom  |
| :---: | :---: | :---: | :---: | :---: | :---: | 
| `/`    |  ។    |      | ฯ     |  |  | 
| `//`    |  ៕    |      |      |  |  | 
| `%`    | ៚     |      | ៚     |  |  | 
| `|`    | (space)     |      |      |  |  | 
| `-`    | (implicit syllable break)     |      |      |  |  | 
| (space)    | (implicit word break)     |      |      |  |  | 
| ?    | ៙     |      |      |  |  | 
| ?    | ៗ     |      |      |  |  | 
| ?    | ៖     |      |      |  |  | 


## Numerals

| twktt | Khmer | Lao   | Thai  | Tham  | Khom  |
| :---: | :---: | :---: | :---: | :---: | :---: | 
| `0`    | ០     |      |      |  |  | 
| `1`    | ១     |      |      |  |  | 
| `2`    | ២     |      |      |  |  | 
| `3`    | ៣     |      |      |  |  | 
| `4`    | ៤     |      |      |  |  | 
| `5`    | ៥     |      |      |  |  | 
| `6`    | ៦     |      |      |  |  | 
| `7`    | ៧     |      |      |  |  | 
| `8`    | ៨     |      |      |  |  | 
| `9`    | ៩     |      |      |  |  | 

